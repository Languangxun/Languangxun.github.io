...
| `factor_lab/` | 因子级消融框架（21 原子因子、**面板窗口 2000 交易日**（2026-09-25 由 1000 提升）、2^21 穷举、BH-FDR 过拟合治理）+ 筹码特征；结论为负面证据（pass=0） |
...
- **拉取深度可调（v6.1.4 热修③）**：`CFG.MAX_FETCH_BARS`（ini `[predict] max_fetch_bars`，
  ...
  `--backfill` 深历史回填仍按 ≥950 根目标翻页（`_bf_fetch_one` 不受该值影响）。
...
- **ETF 历史回填**：`backfill_etf_history()` 走主库 hfq 路径 + `_sync_adjust`，断点续传（≥1000 根且新鲜则跳过）、6 线程、全局节流；
  实测（2026-09-25 新浪兜底恢复）：代码表 **1796 只** → 有 K 线 **1500**（613 只 1000+ 根），合计 **112 万根**；
